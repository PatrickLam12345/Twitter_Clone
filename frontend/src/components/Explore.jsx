...
export default function Explore() {
  ...
  const getTweets = async () => {
    try {
      const response = await axios.get(
        "https://twitterclone2024.onrender.com/api/user/getMoreTweets",
        {
          headers: {
  ...
  const getUsers = async () => {
    try {
      const response = await axios.get(
        "https://twitterclone2024.onrender.com/api/user/getMoreUsers",
        {
          headers: {
            authorization: window.localStorage.getItem("token"),
          },
  ...
  return (
    <div>
      <div>
        <input
          placeholder="Search (type user to find seeded users)"
          value={searchQuery}
